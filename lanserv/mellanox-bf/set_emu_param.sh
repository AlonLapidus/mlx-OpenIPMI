...
# Get the OOB port interface information
# $1 holds the port index
get_oob() {
	get_port_info_cmd="ip -s link"
	oob=$($get_port_info_cmd | grep -o "oob*$1" | head -1)
	echo "LAN Interface:" > $EMU_PARAM_DIR/oob$1
...
# eth_and_ib will be true if board has a Eth configured port and a IB configured port
	...
	get_connectx_net_info "0" $eth_and_ib "eth"
	get_connectx_net_info "1" $eth_and_ib "eth"
	get_connectx_net_info "0" false "oob"
fi
